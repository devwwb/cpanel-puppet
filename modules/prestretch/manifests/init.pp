class prestretch (
  ...
  if $enabled {
    ...
    exec { 'set default facts classifier':
      command   => "/bin/bash -c '$directory/set_default_facts_classifier.sh'",
      logoutput => true,
    } ->
    exec { 'backup mysql':
